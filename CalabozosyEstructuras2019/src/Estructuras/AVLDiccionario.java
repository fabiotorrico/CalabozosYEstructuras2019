 ...
 * @author Torrico
 */
public class AVLDiccionario {

    private NodoAVLDicc raiz;

    public AVLDiccionario() {
        this.raiz = null;
    }

    public Object obtenerDato(Comparable clave) {
        Object dato = null;
        NodoAVLDicc aux = obtenerDatoAux(this.raiz, clave);
    ...
    private NodoAVLDicc obtenerDatoAux(NodoAVLDicc nodo, Comparable clave) {

        NodoAVLDicc datoAux = null;
    ...
    private boolean insertarAux(NodoAVLDicc nodo, Comparable clave, Object dato) {
        ...
        if (clave.compareTo(nodo.getClave()) < 0) {
            if (nodo.getIzq() == null) {//si no tiene hijo izquierdo agrego
                nodo.setIzq(new NodoAVLDicc(clave, dato));
                exito = true;
            ...
            if (clave.compareTo(nodo.getClave()) > 0) {
                if (nodo.getDer() == null) {// si no hay hijo derecho agrego
                    nodo.setDer(new NodoAVLDicc(clave, dato));
                    exito = true;
    ...
    private boolean eliminarAux(NodoAVLDicc nodo, Comparable clave, NodoAVLDicc padre) {
        ...
        if (nodo.getClave().compareTo(clave) == 0) {//encontre el nodo a eliminar
            ...
            if (nodo.getDer() == null && nodo.getIzq() == null) {
                ...
                NodoAVLDicc hijoDer = nodo.getDer();
                NodoAVLDicc hijoIzq = nodo.getIzq();// casoa hijo izquierdo y derecho del padre
    ...
    private NodoAVLDicc eliminarCaso3aux(NodoAVLDicc n, NodoAVLDicc padre) {
    ...
    private NodoAVLDicc rotacionSimpleIzquierda(NodoAVLDicc pivote) {
        NodoAVLDicc h, temp;
        h = pivote.getDer();
        temp = h.getIzq();
        h.setIzq(pivote);
    ...
    private NodoAVLDicc rotacionSimpleDerecha(NodoAVLDicc pivote) {
        NodoAVLDicc h, temp;
        h = pivote.getIzq();
        temp = h.getDer();
        h.setDer(pivote);
    ...
    private void listarAux3(NodoAVLDicc n, Lista nLista) {
        if (n != null) {
    ...
    private void listarAux2(NodoAVLDicc n, Lista nLista, String subcadena) {
        if (n != null) {
    ...
    private Lista listarAux(NodoAVLDicc n, Lista l1) {
    ...
    private Lista listarRangoAux(NodoAVLDicc n, Lista l1, Comparable min, Comparable max) {
